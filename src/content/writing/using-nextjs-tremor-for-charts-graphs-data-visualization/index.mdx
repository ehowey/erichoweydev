...
Under the hood Tremor is using a library called [Recharts](https://recharts.org/en-US) for the charts. The custom tooltip in Tremor is an extension of the [custom tooltip API](https://recharts.org/en-US/examples/CustomContentOfTooltip) that Recharts uses. We can also use their types package to get the types we need for our custom tooltip.

```
npm install --save -D @types/recharts
```
