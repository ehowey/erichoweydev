...
excerpt: Charts, graphs, and data visualizations are an important tool when presenting data or trying to explain a complex topic more clearly. They are widely used in websites, apps, and other digital media. Just think back to the last major election and I guarantee it was brimming with charts and graphs!
...
That isn't to say it is perfect, by being opinionated about design Tremor is less configurable than some other charting libraries. Tremor also uses Tailwind for styling which is a bonus for some and a dealbreaker for others. Under the hood Tremor is using a library called [Recharts](https://recharts.org/en-US) for the charts.
...
There are lots of way you could approach this customization, here is one approach. Note that this example includes the types for the custom tooltip as well. The two props to pay attention to here are the `label` and the `payload`. The label is linked with the index value from Tremor. The payload is linked to the category values from Tremor. It can be worth quickly console logging these values because the payload varies slightly between chart types.
...
Charts can be resource intensive to load on the client and using [Next.js dynamic imports](https://nextjs.org/docs/pages/building-your-application/optimizing/lazy-loading) to help decrease the time to interactive is an option. Behind the scenes Next.js uses React.lazy() and the Suspense API to show an initial loading state and then the chart after it has loaded.
...
This one is a pretty simple, but important tip. If your charts will be contained within a block of prose you will want to use `not-prose` on a wrapper around your chart.
...
Tremor is slowly expanding its library of components with the goal of being the _React library to build dashboards fast_. My personal experience has been that it has some really handy utility components like `<Title>` and `<Subtitle>` that make prettying up your charts very easy to do.

It also gives you some nice progress trackers, inputs, and helper components that are not specifically a chart but help when trying to visualize data or build dashboards.

My experience has been that Tremor hits a sweet spot of configurability and happy defaults. There are lower level charting libraries out there that are less opinionated and give you more control. There are also a whole bunch of no-code SAAS tools for charting and graphs that give you less control.

I hope this has helped. Happy coding and happy charting!
