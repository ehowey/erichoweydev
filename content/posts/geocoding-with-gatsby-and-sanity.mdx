...
For the purpose of this tutorial we will assume you are building something like a real estate app - where you would need to map homes based on their address. But a realtor (or other conten) is not going to know the lat/long of a home - we need to handle that for them.

> There is a plugin called [gatsby-transformer-opencage-geocoder](https://www.gatsbyjs.org/packages/gatsby-transformer-opencage-geocoder/) which wasn't working for me. I did however take inspiration from the plugin for how to do this.
...
Next, if you haven't already done so, you need to setup a SANITY.io content structure. Here is what a really basic content structure for a home could look like.
...
export default {
  ...
  fields: [
    ...
    },
    {
      ...
      validation: (Rule) =>
        Rule.custom((postal) => {
          const regex = /([ABCEGHJ-NPRSTVXY]\d[A-Z])[\s\-]?(\d[A-Z]\d)/gi // Make sure it actually a postal code
...
Last but not least, we are going to wrap everything in a try...catch statement to give us better error logging. I have also included the complete code for the entire process in this snippet so you can see all the steps put together.
