...
I am actively using the themes in production on a number of projects and am gradually making improvements to the themes as I run into issues or problems. I am excited to say that my central philosophy of having a core theme that is then extended with child and sibling themes is working really well.

One of the biggest successes for me is that I can very quickly bootstrap myself into working prototypes and then graudally use component shadowing and variants to customize a project as it becomes more polished.

Here are a few of things I have planned for `gatsby-theme-catalyst` in 2021.

## Moving to unified versioning

Currently I am using indepedent versioning with these packages, so one package might be at v3.0.0 while another package is at v5.0.0. This gets confusing and makes it appear that different parts of the themes and starters do not work together when actually they do. With this in mind I am going to move to unified versioning early in 2021 which should help make it more clear that the themes all work together. The tradeoff is that there might be more frequent version changes as the package versioning will now be linked.
...
I was excited to see that Wes Bos picked exactly the same tech stack I did for [his latest course](https://mastergatsby.com/); Gatsby, SANITY and Netlify. I have now built a few projects using this stack and can't see myself changing anytime soon. I working on integration with [sanity.io/create](https://create.sanity.io/), creating a better default dashboard experience for the themes, and possibly implementing some previews into the content studios.

## Continued refinement of existing themes

As it stands now I am really happy with where most of the themes are, but also recognize there are still many areas for improvement. For example the dropdown menus for `header-bigtop` are very basic and not as refined as what is available in `header-top`. I will also be continuing to make accessibility improvements to the core layout themes so that Gatsby Theme Catalyst is as accessible as possible.

## Transition to typescript

This one might be a stretch for me - time will tell. I am hopeful that I can begin transitioning this project to typescript over the coming year to make the themes easier to use for developers. I will start with the core theme, and then focus on areas of the codebase where it will make the most difference.
