...
> We are using v3.0 of react-leaflet which was released at the end of 2020 - this code won't work for v2.x.x but I code for v2 of react-leaflet so feel free to ask me questions.
...
- I assume you have a basic familiarity with Gatsby and React hooks - but do my best to walk you through things. There just isn't space however to explain what hooks like `useState` or `useEffect` do.

- I focuse almost entirely on the javascript side of things in this post so what we build ain't pretty!
...
We are going to setup a quick starter project, add our dependencies, display a basic map and then add some imaginary placeholder data. In a real world application your data would be coming from an API/backend of some kind, but that is beyond the scope of this post.
...
Save and refrsh your browser, you should see our new markers on the map. The GeoJson component automatically creates the markers for us - now we are getting somewhere!
...
> This code is for react-leafet v3.0 which was a fairly major change from v2.x.x and involves a switch to a hooks based API.
...
We are going to add our logic to seperate functional components, e.g. `GetVisibleMarkers` and to the map page itself. Here is an outline of what that final page structure will look like once we complete all the steps below. You might find it helpful to [reference the code on GitHub](https://github.com/ehowey/gatsby-advanced-mapping) and this post simultaneously to see the code snippets in context of the larger project.
...
In the demo I created some very rudimentary buttons to simulate this, but in a production application you could handle this in a number of different ways depending on your needs. I have used [FuseJS](https://fusejs.io/) succesfully for this in previous projects.
...
```shell
yarn install leaflet.locatecontrol
```
...
If you are looking for more reading [Colby Fayock](https://www.colbyfayock.com/) has done a lot of writing on mapping in React and Gatsby in particular. He has a [great course on mapping with Egghead.io](https://egghead.io/playlists/mapping-with-react-leaflet-e0e0) as well which I found really helpful in my mapping journey.
