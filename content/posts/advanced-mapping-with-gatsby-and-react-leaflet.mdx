...
categories: [Gatsby]
date: 2020-12-30
featuredImage: ../post-assets/featured-advanced-mapping.jpg
socialImage: ../post-assets/social-advanced-mapping.jpg
draft: true
---

Hang on to your hats - this is a blustery one! In this post we walk through setting up an imaginary real estate mapping app using Gatsby and [React Leaflet](https://react-leaflet.js.org/).

> We are using v3.0 of react-leaflet which was recently released at the end of 2020 - this code won't work for v2.x.x but I have done these same tasks for v2 of react-leaflet so feel free to ask me questions.

You can checkout the [finished code on GitHub](https://github.com/ehowey/gatsby-advanced-mapping) and a [live demo](https://gatsby-advanced-mapping.netlify.app/) of what we will be building. If you want to see a production version of this code you can poke around [www.mindmapbc.ca/map](https://www.mindmapbc.ca/map/).

A few notes before we get going. I assume you have a basic familiarity with Gatsby and React hooks - but do my best to walk you through things step by step. You should also know that I am focusing entirely on the javascript in this post so what we build ain't pretty! Finally if you are already familiar with react-leaflet you can probably skip straight to part 2 where we begin covering the more advanced mapping topics.

## Part 1: Initial map setup

We are going to setup a quick starter project, add our dependencies, display a basic map and then add some imaginary data. In a real application your data would most likely be coming from an API of some kind, but that is beyond the scope of this post.

### Step 1: Installation and dependencies

Here we are using [Gatsby Theme Catalyst](https://www.gatsbycatalyst.com/) (but you can use any starter you want) to quickly bootstrap a working project.
...
There are two gotchas here that are worth noting. The first is that because Gatsby is statically rendered at build time you need to check that the component has mounted before rendering the map to the DOM. This is what our [useHasMounted hook](https://www.joshwcomeau.com/snippets/react-hooks/use-has-mounted/) does for us. The second gotcha is that the map needs to have a height, if you are miss the height for some reason you will be staring at a blank screen.
...
To add our imaginary real estate data to the map we will use the GeoJson component from react-leaflet which we will import and use to replace the Marker and Popup components. Here is what your `map.js` file should look like now.
